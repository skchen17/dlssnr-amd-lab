#pragma once
// Dynamic-resolution, externally weighted residual network on a private list.
...
inline std::array<float,256> LoadResidualModel(const std::filesystem::path& path) {
    std::ifstream stream(path);
    std::string magic;
    size_t count=0;
    if(!(stream>>magic>>count)||magic!="DLSSNR_DYNAMIC_RESIDUAL_V1"||count!=256)
        throw std::invalid_argument("network model header");
    ...
    while(std::getline(stream,line)) {
        ...
        size_t index=0;float value=0;
        if(!(fields>>index>>value)||index>=weights.size()||assigned[index]||!std::isfinite(value))
            throw std::invalid_argument("network model entry");
        std::string trailing;if(fields>>trailing)throw std::invalid_argument("network model trailing field");
        assigned[index]=true;weights[index]=value;
    }
    if(weights[252]<=0.0f||weights[252]>1.0f)
        throw std::invalid_argument("network model residual scale");
    return weights;
}

inline std::array<float,256> CalibrationResidualModel() {
    std::array<float,256> w{};
    constexpr int offsets[5]={1,3,4,5,7};
    constexpr float coefficients[5]={-1,-1,4,-1,-1};
    for(int i=0;i<5;++i)for(int c=0;c<3;++c){
        w[(offsets[i]*3+c)*4+c]=coefficients[i];
        w[108+(offsets[i]*3+c)*4+c]=-coefficients[i];
    }
    for(int c=0;c<3;++c){w[224+c*4+c]=1;w[236+c*4+c]=-1;}
    w[252]=0.03125f;
    return w;
}

class OutputNetwork {
    ComPtr<ID3D12Device> device_;ComPtr<ID3D12Resource> output_,scratch_,before_,after_,model_;
...
public:
    OutputNetwork(ID3D12Device* device,ID3D12Resource* output,const std::vector<uint8_t>& shader,
                  const std::array<float,256>& weights=CalibrationResidualModel(),UINT64 budget=128*1024*1024)
        :device_(device),output_(output){
        if(!device||!output||shader.empty()||shader.size()>1024*1024)throw std::invalid_argument("network null/invalid input");
        for(float value:weights)if(!std::isfinite(value))throw std::invalid_argument("network non-finite weight");
        ComPtr<ID3D12Device> owner;Check(output->GetDevice(IID_PPV_ARGS(&owner)));if(!ffx_capture::Same(owner.Get(),device))throw std::invalid_argument("network device mismatch");
        ...
        if(d.Dimension!=D3D12_RESOURCE_DIMENSION_TEXTURE2D||d.MipLevels!=1||d.DepthOrArraySize!=1||d.SampleDesc.Count!=1||
           ...
           (d.Flags&(D3D12_RESOURCE_FLAG_ALLOW_SIMULTANEOUS_ACCESS|D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE)))throw std::invalid_argument("network unsupported output");
        device->GetCopyableFootprints(&d,0,1,0,&footprint_,&rows_,&rowBytes_,&bytes_);if(!bytes_||bytes_>budget/2||rowBytes_!=UINT64(width_)*8||rows_!=height_)throw std::invalid_argument("network byte/shape budget");
        ...
        D3D12_HEAP_PROPERTIES upload{};upload.Type=D3D12_HEAP_TYPE_UPLOAD;b.Width=sizeof(weights);
        Check(device->CreateCommittedResource(&upload,D3D12_HEAP_FLAG_NONE,&b,D3D12_RESOURCE_STATE_GENERIC_READ,nullptr,IID_PPV_ARGS(&model_)));
        void* mapped=nullptr;D3D12_RANGE none{0,0};Check(model_->Map(0,&none,&mapped));std::memcpy(mapped,weights.data(),sizeof(weights));model_->Unmap(0,&none);
        ...
        D3D12_DESCRIPTOR_RANGE ranges[2]{};D3D12_ROOT_PARAMETER params[3]{};
        ...
        D3D12_ROOT_SIGNATURE_DESC rd{};rd.NumParameters=3;rd.pParameters=params;ComPtr<ID3DBlob> blob,error;Check(D3D12SerializeRootSignature(&rd,D3D_ROOT_SIGNATURE_VERSION_1,&blob,&error));Check(device->CreateRootSignature(0,blob->GetBufferPointer(),blob->GetBufferSize(),IID_PPV_ARGS(&root_)));
    ...
    ~OutputNetwork(){if(recorded_&&!retired_)std::terminate();}
    void Record(const D3D12_RESOURCE_BARRIER& forwarded){
        ...
        if(recorded_||forwarded.Type!=D3D12_RESOURCE_BARRIER_TYPE_TRANSITION||forwarded.Flags!=D3D12_RESOURCE_BARRIER_FLAG_NONE||forwarded.Transition.pResource!=output_.Get()||
           ...
           (forwarded.Transition.StateBefore!=D3D12_RESOURCE_STATE_UNORDERED_ACCESS&&forwarded.Transition.StateBefore!=D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE)||forwarded.Transition.StateAfter!=readable)
            throw std::invalid_argument("network requires exact complete output transition");
        ...
        ID3D12DescriptorHeap* heaps[]={descriptors_.Get()};list_->SetDescriptorHeaps(1,heaps);list_->SetComputeRootSignature(root_.Get());list_->SetPipelineState(pipeline_.Get());auto gpu=descriptors_->GetGPUDescriptorHandleForHeapStart();auto step=device_->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);list_->SetComputeRootDescriptorTable(0,gpu);gpu.ptr+=step;list_->SetComputeRootDescriptorTable(1,gpu);list_->SetComputeRootConstantBufferView(2,model_->GetGPUVirtualAddress());list_->Dispatch((width_+7)/8,(height_+7)/8,1);
