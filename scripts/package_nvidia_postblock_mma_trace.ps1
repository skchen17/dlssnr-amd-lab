[CmdletBinding()]
param([string]$TraceRoot = 'results\20260904_750000_postblock_exact_mma_trace')
...
$stage = Join-Path $repo "deliverables\postblock_mma_trace_reference_$stamp"
...
Copy-Item -LiteralPath (Join-Path $trace 'post_block_mma_trace_rtx_v87.ptx') -Destination (Join-Path $payload 'post_block_mma_trace.ptx')
...
Copy-Item -LiteralPath (Join-Path $repo 'POSTBLOCK_MMA_TRACE_REFERENCE_README.md') -Destination (Join-Path $stage 'README.md')
$hashes = [ordered]@{}
...
[ordered]@{
    schema = 1
    package_revision = 'v1_postblock_exact_frame1_selected_cta_mma_trace'
    experiment = 'rtx_postblock_exact_frame1_selected_cta_mma_trace_package'
    result_stem = 'postblock_mma_trace'
    result_experiment = 'rtx_postblock_exact_frame1_selected_cta_mma_trace'
    result_classification = 'RTX_POSTBLOCK_FP8_F16_MMA_ORACLE'
    ptx_file = 'post_block_mma_trace.ptx'
