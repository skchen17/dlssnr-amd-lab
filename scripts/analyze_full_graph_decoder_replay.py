#!/usr/bin/env python3
"""Analyze decoder exact-state graph-edge replay on RX 9070 XT."""
...
def analyze(root: Path) -> dict:
    ...
    state_gate = (len(model) == int(manifest["model_arena"]["bytes"])
                  and sha256(model) == manifest["model_arena"]["sha256"])
    execution_gate = numerical_gate = determinism_gate = True
    reports = {}
    settled_edges = 0
    exact_edges = 0
    final_image_gate = False
    e4m3_code_advisory_gate = True
    advisories = []
    ...
    for case in manifest["slots"]:
        ...
        execution = all(
            ...
            and probe.get("n1_arena_param_view_count") == len(case["activation_param_views"])
            and probe.get("n1_weight_param_view_count") == len(case["weight_param_views"])
        ...
        for output in case["outputs"]:
            ...
            if output["tensor_type"] == "rgba16f":
                comparison = compare_rgba16f_rgb(reference, candidate)
            elif output["tensor_type"] == "fp16":
                comparison = compare_fp16(reference, candidate)
            else:
                comparison = compare_e4m3(reference, candidate)
            if output["tensor_type"] == "e4m3":
                code_advisory = comparison["exact_or_adjacent_fraction"] >= 0.95
                e4m3_code_advisory_gate &= code_advisory
                numerical_pass = (
                    comparison["nonfinite_pairs"] == 0
                ...
                comparison["parity_gate"] = {
                    ...
                    "exact_or_adjacent_advisory_pass": code_advisory,
                    "rationale": (
                        "Decoded numerical correlation and NRMSE are normative; "
                        "E4M3 code adjacency remains visible because encoding "
                        "distance is not a linear numerical-error metric."),
                }
                if not code_advisory:
                    advisories.append(
                        f"slot {slot} E4M3 exact-or-adjacent fraction "
    ...
    passed = (execution_gate and state_gate and numerical_gate and
              determinism_gate and final_image_gate)
    return {
        "schema": 1,
        "experiment": "decoder_slots99_154_exact_state_rtx_vs_rx9070xt",
        "status": "PASS" if passed else "FAIL",
        "classification": "SETTLED_GRAPH_EDGE_AND_FINAL_IMAGE_CROSS_VENDOR_PARITY",
        "counts_as_s7": False,
        "execution_gate": execution_gate,
        "exact_state_gate": state_gate,
        "settled_numerical_gate": numerical_gate,
        "amd_repeat_determinism_gate": determinism_gate,
        "final_rgba16f_image_gate": final_image_gate,
        "e4m3_code_advisory_gate": e4m3_code_advisory_gate,
        "settled_edge_count": settled_edges,
        "bitwise_exact_settled_edge_count": exact_edges,
        "advisories": advisories,
        "slots": reports,
        "limitations": [
            "function-isolated exact-state replay, not yet one integrated 156-slot launch",
            "surface output uses the explicit linear RGBA16F compatibility ABI",
            "RGBA16F parity is evaluated on RGB only; constant alpha is excluded from the normative metrics",
        ],
        "next_gate": "one integrated 156-slot RX 9070 XT frame with final image export",
    }
