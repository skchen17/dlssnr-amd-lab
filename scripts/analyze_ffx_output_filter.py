"""Validate the dynamic-resolution five-tap GPU residual plumbing stage."""
import argparse
...
def analyze(root: Path, live=True):
    ...
    if tuple(event.get('event') for event in markers) != MARKERS:
        raise ValueError('filter lifecycle order/count')
    returns = [event for event in events if event.get('event') == 'output_boundary_return']
    accepted = [event for event in returns
                if event.get('batch_supported') is True and event.get('lifecycle_match') is True]
    if len(accepted) != 1:
        raise ValueError('accepted output boundary count')
    selected = [markers[0], accepted[0], *markers[1:]]
    positions = [events.index(event) for event in selected]
    if positions != sorted(positions) or len({event.get('window') for event in selected + returns}) != 1:
        raise ValueError('filter lifecycle window/order')
    arm, _, recorded, submitted, complete = selected
    if (recorded.get('isolated_host_only') is live or submitted.get('isolated_host_only') is live or
            ...
            recorded.get('gpu_residual_filter_recorded') is not True or
            ...
            submitted['submission_suffix_lists'] < 1 or
            complete.get('fence_completed') is not True or
            complete.get('gpu_residual_filter') is not True or
            complete.get('filter_kind') != 'dynamic_residual_5tap' or
            complete.get('patch_rect') is not None or
            complete.get('trained_weights') is not False or
            complete.get('game_frame_capture_verified') is not live or
            complete.get('nr_verified') is not False):
        raise ValueError('filter provenance')
    ...
    if (type(width) is not int or type(height) is not int or not 1 <= width <= 8192 or
            ...
            metadata.get('raw_bytes') != width * height * 8 or
            metadata.get('fence_completed') is not True or
            metadata.get('write_back_performed') is not True or
            metadata.get('replacement_pixels_supplied') is not True or
            metadata.get('patch_rect') is not None or
            metadata.get('filter_kind') != 'dynamic_residual_5tap' or
            metadata.get('gpu_residual_filter') is not True or
            metadata.get('trained_weights') is not False or
            metadata.get('game_frame_capture_verified') is not live or
            metadata.get('nr_verified') is not False):
        raise ValueError('filter metadata')
    ...
    for offset in range(0, len(after), 8):
        old = struct.unpack_from('<4e', before, offset)
        new = struct.unpack_from('<4e', after, offset)
    ...
    if not changed_pixels or not changed_components or not alpha_exact or not max(rgb_abs):
        raise ValueError('filter did not produce valid residual change')
    if live:
    ...
    return dict(
        status='GAME_GPU_RESIDUAL_FILTER_PASS' if live else 'ISOLATED_GPU_RESIDUAL_FILTER_PASS',
        width=width, height=height, raw_bytes=len(after),
        before_sha256=hashlib.sha256(before).hexdigest(),
        after_sha256=hashlib.sha256(after).hexdigest(),
        changed_pixels=changed_pixels, changed_components=changed_components,
        alpha_exact=True, mean_abs_rgb_change=math.fsum(rgb_abs) / len(rgb_abs),
        max_abs_rgb_change=max(rgb_abs), submission_batch_count=submitted['submission_batch_count'],
        submission_batch_index=submitted['submission_batch_index'],
        submission_suffix_lists=submitted['submission_suffix_lists'],
        internal_hook_bypass=True, queue_wait_gate_used=False,
        game_frame_capture_verified=live, game_writeback_verified=live,
        output_replacement_verified=live, gpu_residual_filter=True,
        trained_weights=False, reconstructed_network_output=False, dlss_nr_verified=False)
