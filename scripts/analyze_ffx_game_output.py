"""Validate one fenced, output-only FFX frame copied from a live GoWR process."""
import argparse
...
def analyze(root: Path):
    ...
    if tuple(e.get('event') for e in markers) != MARKERS:
        raise ValueError('output lifecycle order/count')
    ...
    if len({e.get('window') for e in selected + returns}) != 1 or selected[0].get('window', 0) < 1:
        raise ValueError('output lifecycle window')
    ...
    if (recorded.get('isolated_host_only') is not False or submitted.get('isolated_host_only') is not False or
            recorded.get('write_back_performed') not in (None, False) or submitted.get('write_back_performed') not in (None, False) or
            ...
            complete.get('fence_completed') is not True or complete.get('nr_verified') is not False):
        raise ValueError('game/fence provenance')

    output = root / 'output_boundary'
    ...
    if (type(width) is not int or type(height) is not int or not 1 <= width <= 8192 or not 1 <= height <= 8192 or
            ...
            metadata.get('write_back_performed') not in (None, False) or metadata.get('replacement_pixels_supplied') not in (None, False) or
    ...
    for pixel in range(0, pixel_count, stride):
        offset = pixel * 8
        values = struct.unpack_from('<4e', raw, offset)
    ...
    return dict(status='GAME_OUTPUT_CAPTURE_PASS', width=width, height=height, format=fmt,
                ...
                source='existing game FFX output texture after provider dispatch',
                game_frame_capture_verified=True, output_replacement_performed=False,
