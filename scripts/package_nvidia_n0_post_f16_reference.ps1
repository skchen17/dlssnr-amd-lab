...
$sources = [ordered]@{
    ...
    'n0_post_f16.ptx' = Join-Path $repo 'results\20260831_134000_amd_n0_post_f16_checkpoint\n0_post_f16_original.ptx'
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'run_nvidia_n0_post_f16_reference.ps1') -Destination $scripts
Copy-Item -LiteralPath (Join-Path $repo 'N0_POST_F16_PACKAGE_README.md') -Destination (Join-Path $stage 'README.md')
$hashes = [ordered]@{}
foreach ($name in $sources.Keys) { $hashes[$name] = (Get-FileHash -LiteralPath (Join-Path $payload $name) -Algorithm SHA256).Hash }
[ordered]@{
    schema = 1
    experiment = 'rtx_n0_post_f16_checkpoint_package'
    status = 'READY'
    classification = 'USER_OWNED_INTEROPERABILITY_PACKAGE'
    counts_as_s6 = $false
    expected_gpu = 'RTX 50 series / SM120'
    checkpoint = 'immediately after first 16 f16 MMA operations'
    grid = @(1,1,1)
    block = @(32,1,1)
    bytes_per_lane = 128
    lanes = 32
