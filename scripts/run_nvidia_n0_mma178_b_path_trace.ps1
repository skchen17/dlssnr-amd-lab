...
$required = @('neural_reference_probe.exe','n0_mma178_b_path_trace.ptx','n0_original.ptx','input_rgba16f.raw','weights.raw','params.raw')
...
$result = Join-Path $root "_n0_mma178_b_path_trace_reference_result_$stamp"
...
& (Join-Path $payload 'neural_reference_probe.exe') --nvcuda $nvcuda `
    --ptx (Join-Path $payload 'n0_mma178_b_path_trace.ptx') `
...
$trace = Join-Path $result 'mma178_b_path_trace.raw'; $traceBytes = [int]$packageManifest.trace_bytes; $traceOffset = [int64]$packageManifest.trace_offset
...
[ordered]@{
    schema=1;experiment='rtx_n0_mma178_b_path_trace';status=if($pass){'PASS'}else{'FAIL'}
    classification='RTX_N0_ZERO_INPUT_MMA178_B1_PATH_ORACLE';counts_as_s7=$false
