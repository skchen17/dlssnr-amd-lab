...
if (-not (Test-Path -LiteralPath $nvcuda)) {
    throw "NVIDIA driver not found: $nvcuda"
}
...
$work = Join-Path $root "_n0_normal2_full_grid_traces_work_$stamp"
$result = Join-Path $root "_n0_normal2_full_grid_traces_reference_result_$stamp"
...
function Invoke-Probe([string]$Ptx, [string]$Tag, [int]$ExtraBytes) {
    ...
    $arguments = @(
        '--nvcuda', $nvcuda,
        '--ptx', $Ptx,
        '--function', $packageManifest.function,
        '--json', $json,
        ...
        '--n0-output', $output,
        '--n0-grid-x', ([string]$packageManifest.grid[0]),
        '--n0-grid-y', ([string]$packageManifest.grid[1])
    )
    if ($ExtraBytes -gt 0) {
        $arguments += @('--n0-scratch-extra-bytes', [string]$ExtraBytes)
    }
...
foreach ($variant in $packageManifest.variants) {
    ...
    $rows += [ordered]@{
        ...
        reference_output_preserved = $preserved
        trace_filename = (Split-Path -Leaf $tracePath)
...
$resultManifest = [ordered]@{
    schema = 1
    experiment = 'rtx_n0_normal2_full_grid_traces'
    status = if ($pass) { 'PASS' } else { 'FAIL' }
    classification = 'N0_SECOND_BOX_MULLER_PATH_FULL_GRID_DATASET'
    counts_as_s7 = $false
    payload_integrity = $integrity
    device_name = [string]$baselineProbe.device_name
    grid = @($packageManifest.grid)
    block = @($packageManifest.block)
    sample_count = [int]$packageManifest.sample_count
    ...
    baseline_output_sha256 = $baselineHash
    variants = $rows
}
$resultManifest | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $result 'manifest.json') -Encoding utf8
$zip = "$result.zip"
