[CmdletBinding()]
param([string]$TraceRoot = 'results\20260901_040000_slot3_cta2_f16_path_trace')
...
$stage = Join-Path $repo "deliverables\slot3_f16_path_trace_reference_$stamp"
...
$sourcePtx = Join-Path $trace 'slot3_f16_path_original.ptx'
...
[IO.File]::WriteAllText((Join-Path $payload 'slot3_f16_path_trace.ptx'), $normalized, [Text.UTF8Encoding]::new($false))
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'run_nvidia_slot3_f16_path_trace.ps1') -Destination $scripts
Copy-Item -LiteralPath (Join-Path $repo 'SLOT3_F16_PATH_TRACE_REFERENCE_README.md') -Destination (Join-Path $stage 'README.md')
$required = @('neural_reference_probe.exe','slot3_f16_path_trace.ptx','activation_arena_before.raw','output_reference.raw','model_arena.raw','params.raw')
$hashes = [ordered]@{}
...
[ordered]@{
    schema = 1
    experiment = 'rtx5070_slot3_f16_path_register_trace_package'
