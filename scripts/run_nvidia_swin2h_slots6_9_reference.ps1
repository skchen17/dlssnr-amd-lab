...
$cases = @(
    [ordered]@{slot=6;ptx='inpview.ptx';function='cc_tinlayout_fused_swin_2h_64_2_inpview_tilesync_fp8';grid=@(20,12,1);releases=240;wait=$false;release=$true;extra=$false},
    [ordered]@{slot=7;ptx='chained.ptx';function='cc_tinlayout_fused_swin_2h_64_2_chained_fp8';grid=@(21,13,1);releases=273;wait=$true;release=$true;extra=$false},
    [ordered]@{slot=8;ptx='chained.ptx';function='cc_tinlayout_fused_swin_2h_64_2_chained_fp8';grid=@(21,12,1);releases=252;wait=$true;release=$true;extra=$false},
    [ordered]@{slot=9;ptx='ds_wait.ptx';function='cc_tinlayout_fused_swin_2h_64_2_ds_wait_fp8';grid=@(20,13,1);releases=0;wait=$true;release=$false;extra=$true}
)
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$result = Join-Path $root "_swin2h_slots6_9_reference_result_$stamp"
...
foreach ($case in $cases) {
    ...
    $args = @(
        ...
        '--n1-block-x','32','--n1-block-y','2','--n1-expected-releases',[string]$case.releases
    )
    ...
    $records += [ordered]@{
        slot=$slot;grid=$case.grid;block=@(32,2,1);exit_code=$exitCode
...
[ordered]@{schema=1;experiment='rtx_swin2h_slots6_9_same_input_reference';status=if($pass){'PASS'}else{'FAIL'};classification='RTX_ORIGINAL_PTX_NUMERICAL_REFERENCE';counts_as_s7=$false;payload_integrity=$integrity;payload_sha256=$hashes;slots=$records} |
