...
foreach ($runIndex in 1..2) {
    foreach ($case in $manifest.slots) {
        ...
        foreach ($view in $case.weight_param_views) {
            $arguments += @('--n1-weight-param-view',
                "$($view.param_offset):$($view.weight_offset)")
        }
        & $probe @arguments *> (Join-Path $runDir 'stdout.log')
        if ($LASTEXITCODE -ne 0) {
            throw "RX 9070 XT decoder replay failed at run $runIndex slot $slot"
        }
    }
}

$report = Join-Path $result 'manifest.json'
python (Join-Path $PSScriptRoot 'analyze_full_graph_decoder_replay.py') `
