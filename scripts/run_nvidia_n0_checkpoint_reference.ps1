...
$result = Join-Path $root "_n0_checkpoint_reference_result_$stamp"
...
& (Join-Path $payload 'n0_reference_probe.exe') `
    --nvcuda $nvcuda `
    --ptx (Join-Path $payload 'n0_pre_mma.ptx') `
...
$fragmentPath = Join-Path $result 'pre_mma_fragments.raw'
if (Test-Path -LiteralPath $scratch) {
    $scratchBytes = [System.IO.File]::ReadAllBytes($scratch)
    if ($scratchBytes.Length -ge 3072) {
        $fragments = [byte[]]::new(3072)
        [System.Array]::Copy($scratchBytes, $fragments, 3072)
...
$pass = $exitCode -eq 0 -and $probe -and $probe.pass -and $probe.kernel_launched -and `
    ...
    (Test-Path -LiteralPath $fragmentPath) -and (Get-Item -LiteralPath $fragmentPath).Length -eq 3072
$hashes = [ordered]@{}
foreach ($name in $required) { $hashes[$name] = (Get-FileHash -LiteralPath (Join-Path $payload $name) -Algorithm SHA256).Hash }
[ordered]@{
    schema = 1
    experiment = 'rtx_n0_pre_mma_fragment_checkpoint'
