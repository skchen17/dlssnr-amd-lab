param(
    ...
    [string]$First128 = 'results\20260905_013000_output_head_pipeline_rx9070xt\first128_fp16.raw',
    [string]$Model = 'local_models\decoded_310_8\model_arena.raw',
    [string]$Base = '-',
    [string]$ExpectedAttention = 'results\20260905_013000_output_head_pipeline_rx9070xt\attention_fp16.raw',
    [string]$ExpectedProjected = 'results\20260905_013000_output_head_pipeline_rx9070xt\attention_projected_fp16.raw',
...
if (-not $OutputDirectory) { $OutputDirectory = Join-Path $repo ("results\{0}_output_head_attention_suffix_d3d12_rx9070xt" -f (Get-Date -Format 'yyyyMMdd_HHmmss')) }
elseif (-not [IO.Path]::IsPathRooted($OutputDirectory)) { $OutputDirectory = Join-Path $repo $OutputDirectory }
foreach ($name in @('Qk','V','First128','Model','ExpectedAttention','ExpectedProjected','ExpectedResidual','ExpectedSurface')) {
...
& (Join-Path $PSScriptRoot 'build_all.ps1') -Only output_head_attention_suffix_d3d12
if ($LASTEXITCODE -ne 0) { throw "build failed: $LASTEXITCODE" }
New-Item -ItemType Directory -Force -Path $OutputDirectory | Out-Null
& (Join-Path $repo 'build\output_head_attention_suffix_d3d12.exe') $Qk $V $First128 $Model $Base `
    $ExpectedAttention $ExpectedProjected $ExpectedResidual $ExpectedSurface `
    (Join-Path $OutputDirectory 'attention_fp16.raw') `
    (Join-Path $OutputDirectory 'attention_projected_fp16.raw') `
    (Join-Path $OutputDirectory 'rgba_residual_fp16.raw') `
    (Join-Path $OutputDirectory 'output_rgba16f.raw') `
    (Join-Path $OutputDirectory 'manifest.json')
if ($LASTEXITCODE -ne 0) { throw "D3D12 attention suffix validation failed: $LASTEXITCODE" }
