[CmdletBinding()]
param(
    [string]$TraceRoot = 'results\20260901_123000_n0_norm_path_trace',
    [string]$InputPath,
    [string]$InputVariant = 'captured_rgba16f',
    [string]$PackageTag = 'n0_norm_path_trace'
)
...
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'; $stage = Join-Path $repo "deliverables\${PackageTag}_reference_$stamp"; $payload = Join-Path $stage 'payload'; $scripts = Join-Path $stage 'scripts'
...
$source = Join-Path $traceRoot 'n0_norm_trace_original.ptx'; $text = [IO.File]::ReadAllText($source); $normalized = $text -replace '(?m)^\.version 9\.4$', '.version 8.7'
if ($normalized -eq $text) { throw 'PTX 9.4 version declaration not found' }
[IO.File]::WriteAllText((Join-Path $payload 'n0_norm_path_trace.ptx'), $normalized, [Text.UTF8Encoding]::new($false))
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'run_nvidia_n0_norm_path_trace.ps1') -Destination $scripts
Copy-Item -LiteralPath (Join-Path $repo 'N0_NORM_PATH_TRACE_REFERENCE_README.md') -Destination (Join-Path $stage 'README.md')
$required = @('neural_reference_probe.exe','n0_norm_path_trace.ptx','n0_original.ptx','input_rgba16f.raw','weights.raw','params.raw'); $hashes = [ordered]@{}
foreach ($name in $required) { $hashes[$name] = (Get-FileHash -LiteralPath (Join-Path $payload $name) -Algorithm SHA256).Hash }
[ordered]@{
    schema=1;experiment='rtx_n0_normalization_path_trace_package';source_instrumented_ptx_sha256=(Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash
