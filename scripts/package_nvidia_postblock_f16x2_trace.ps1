[CmdletBinding()]
param([string]$TraceRoot = 'results\20260904_800000_postblock_exact_f16x2_trace')
...
$stage = Join-Path $repo "deliverables\postblock_f16x2_trace_reference_$stamp"
...
Copy-Item -LiteralPath (Join-Path $trace 'post_block_f16x2_trace_rtx_v87.ptx') -Destination (Join-Path $payload 'post_block_f16x2_trace.ptx')
...
Copy-Item -LiteralPath (Join-Path $repo 'POSTBLOCK_F16X2_TRACE_REFERENCE_README.md') -Destination (Join-Path $stage 'README.md')
$hashes = [ordered]@{}
...
[ordered]@{
    schema = 1
    package_revision = 'v1_postblock_exact_frame1_selected_cta_f16x2_trace'
    experiment = 'rtx_postblock_exact_frame1_selected_cta_f16x2_trace_package'
    result_stem = 'postblock_f16x2_trace'
    result_experiment = 'rtx_postblock_exact_frame1_selected_cta_f16x2_trace'
    result_classification = 'RTX_POSTBLOCK_F16X2_ORACLE'
    ptx_file = 'post_block_f16x2_trace.ptx'
    ...
    operation_count = [int]$instrumentation.operation_count
    kind_codes = $instrumentation.kind_codes
