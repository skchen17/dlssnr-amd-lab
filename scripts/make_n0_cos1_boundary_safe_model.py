#!/usr/bin/env python3
"""Select second-cosine correction segments that safely fix normal2 boundaries."""
...
def make(base_model: dict, trace_root: Path) -> dict:
    rtx_boundary = np.fromfile(trace_root / "rtx_normal2_boundary_trace.raw", dtype="<u4").reshape(-1, 2)
    amd_boundary = np.fromfile(trace_root / "amd_normal2_boundary_trace.raw", dtype="<u4").reshape(-1, 2)
    rtx_cos = np.fromfile(trace_root / "rtx_cos1_trace.raw", dtype="<u4").reshape(-1, 2)
    ...
    if not np.array_equal(rtx_cos[:, 0], amd_cos[:, 0]):
        raise ValueError("cos1 inputs differ")
    ...
    return {
        "schema": 1,
        "experiment": f"nvidia_sm120_cos1_boundary_safe_{segments}_segment_degree{degree}",
        "status": "PASS",
        "classification": "FULL_GRID_NORMAL2_BOUNDARY_CONSERVATIVE_COS1_CORRECTION_CANDIDATE",
...
def main() -> int:
    ...
    parser.add_argument(
        "--trace-root",
        type=Path,
        default=repo / "results/20260904_170000_n0_normal2_full_grid_cross_vendor",
    )
    args = parser.parse_args()
    model = make(json.loads(args.base_model.read_text(encoding="utf-8")), args.trace_root)
