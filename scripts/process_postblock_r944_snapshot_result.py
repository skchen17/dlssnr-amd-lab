#!/usr/bin/env python3
"""Strictly ingest the RTX pre-E4 r944 dependency snapshot."""
...
try:
    ...
    from analyze_postblock_r944_snapshot import analyze_bytes
    from instrument_postblock_r944_snapshot import TRACE_BYTES


EXPECTED_REVISION = "v1_postblock_exact_frame1_selected_cta_r944_pre_e4_snapshot"
...
def process(archive: Path, output: Path, amd_trace: Path) -> dict:
    ...
    checks = {
        ...
        "experiment": manifest.get("experiment") ==
            "rtx_postblock_exact_frame1_selected_cta_r944_pre_e4_snapshot",
    ...
    amd = amd_trace.read_bytes(); comparison = analyze_bytes(trace1, amd)
    output.mkdir(parents=True, exist_ok=False)
    (output / "manifest.json").write_bytes(manifest_bytes)
    (output / "rtx_trace.raw").write_bytes(trace1); (output / "amd_trace.raw").write_bytes(amd)
    (output / "comparison.json").write_text(json.dumps(comparison, indent=2) + "\n", encoding="utf-8")
    receipt = {
        "schema": 1, "experiment": "postblock_r944_snapshot_result_ingestion",
        "status": "PASS", "classification": "POSTBLOCK_R944_PRE_E4_SNAPSHOT_ACCEPTED",
        ...
        "first_input_mismatch": comparison["first_input_mismatch"],
...
def main() -> int:
    ...
    parser.add_argument("--amd-trace", type=Path,
                        default=Path("results/20260904_840000_postblock_exact_r944_snapshot/amd_trace.raw"))
