#!/usr/bin/env python3
"""Build full-grid cosine1 and normal2-boundary N0 trace variants."""
...
COS1 = re.compile(r"cos\.approx\.ftz\.f32\s+%r230\s*,\s*%r227\s*;")
SQRT1 = re.compile(r"sqrt\.approx\.ftz\.f32\s+%r225\s*,\s*%r224\s*;")
NORMAL2 = re.compile(r"cvt\.rn\.f16\.f32\s+%rs17\s*,\s*%r151\s*;")
...
def instrument_cos(text: str) -> tuple[str, int]:
    prefix = "__n0_cos1_grid"
    block = "\n".join([
        "{", *address_lines(prefix, 0),
        f"st.global.b32 [%{prefix}_address], %r227;",
        f"st.global.b32 [%{prefix}_address+4], %r230;", "}",
    ])
    return COS1.subn(lambda match: match.group(0) + "\n" + block, text)


def boundary_store(source: str, field_offset: int, half: bool, tag: str) -> str:
    prefix = f"__n0_normal2_grid_{tag}"
...
def instrument_boundary(text: str) -> tuple[str, dict[str, int]]:
    text, sqrt_count = SQRT1.subn(
        lambda match: match.group(0) + "\n" + boundary_store("%r225", 0, False, "sqrt"), text
    )
    text, normal_count = NORMAL2.subn(
        lambda match: match.group(0) + "\n" + boundary_store("%rs17", 4, True, "normal"), text
    )
    return text, {"sqrt1": sqrt_count, "normal2_f16": normal_count}


def generate(text: str, output_dir: Path) -> list[dict]:
    output_dir.mkdir(parents=True, exist_ok=True)
    cos_text, cos_count = instrument_cos(text)
    boundary_text, boundary_counts = instrument_boundary(text)
    if cos_count != 1 or boundary_counts != {"sqrt1": 1, "normal2_f16": 1}:
        raise ValueError(f"instrumentation counts invalid: cos={cos_count}, boundary={boundary_counts}")
    variants = []
    for index, (name, fields, output_text) in enumerate((
        ("cos1", ["phase1_f32", "cos1_f32"], cos_text),
        ("normal2_boundary", ["sqrt1_f32", "normal2_f16_as_u32"], boundary_text),
    )):
        data = output_text.encode("utf-8")
        filename = f"variant_{index}_{name}.ptx"
        (output_dir / filename).write_bytes(data)
        variants.append({
            "index": index, "name": name, "filename": filename, "fields": fields,
...
def main() -> int:
    ...
    report = {
        "schema": 1,
        "experiment": "n0_normal2_full_grid_trace_variants",
        "status": "PASS",
        "classification": "N0_SECOND_BOX_MULLER_PATH_FULL_GRID_DATASET",
        "grid": [GRID_X, GRID_Y, 1], "block": [32, 1, 1],
        "sample_count": SAMPLES, "bytes_per_sample": BYTES_PER_SAMPLE,
        "trace_offset": TRACE_OFFSET, "trace_bytes_per_variant": TRACE_BYTES,
