#!/usr/bin/env python3
"""Trace all N0 FP8 MMA fragments for one selected CTA in a full-grid launch."""
...
def main() -> int:
    ...
    report = {
        "schema": 1, "experiment": "n0_fp8_mma_selected_cta_trace_instrumentation",
        "status": "PASS" if passed else "FAIL",
        "classification": "N0_FULL_GRID_FIRST_OBSERVABLE_MISMATCH_CTA_TRACE",
        "source_sha256": hashlib.sha256(source).hexdigest().upper(),
        "output_sha256": hashlib.sha256(output).hexdigest().upper(),
        "mma_count": count, "lanes": LANES, "bytes_per_lane_mma": BYTES_PER_LANE_MMA,
        "trace_offset": TRACE_OFFSET, "trace_bytes": TRACE_BYTES,
        "scratch_extra_bytes": TRACE_BYTES, "scratch_register": "%rd6",
        "target_cta": [args.target_x, args.target_y, 0],
        "required_grid": [80, 48, 1], "required_block": [32, 1, 1],
    }
