#!/usr/bin/env python3
"""Safely ingest an RTX N0 r2329 reduction-path trace result."""
...
try:
    ...
    from scripts.analyze_n0_reduction_path_trace import analyze
    ...
    from analyze_n0_norm_path_trace import sha256
    from analyze_n0_reduction_path_trace import analyze
...
EXPECTED_EXPERIMENT = "rtx_n0_r2329_reduction_path_trace"
MAX_MEMBERS = 32
...
def inspect_archive(zf: zipfile.ZipFile) -> tuple[zipfile.ZipInfo, zipfile.ZipInfo]:
    ...
    traces = [(path, info) for path, info in files if path.name == "reduction_path_trace.raw"]
    if len(manifests) != 1 or len(traces) != 1:
        raise ValueError("archive must contain one manifest.json and reduction_path_trace.raw")
    ...
    if manifest_info.file_size > MAX_MANIFEST_BYTES or trace_info.file_size != TRACE_BYTES:
        raise ValueError("unexpected manifest or trace size")
    return manifest_info, trace_info


def require_manifest(manifest: dict, trace: bytes) -> None:
    expected_stages = [{"index": index, "name": name, "register": register}
    ...
    checks = {
        ...
        "launch": manifest.get("grid") == [1, 1, 1] and manifest.get("block") == [32, 1, 1]
        and manifest.get("kernel_launched") is True,
        "stages": manifest.get("stage_count") == len(STAGES) and manifest.get("stages") == expected_stages,
        "trace_size": manifest.get("checkpoint_bytes") == TRACE_BYTES,
        "trace_nonzero": manifest.get("checkpoint_nonzero_bytes") == sum(value != 0 for value in trace),
...
def process_archive(archive: Path, amd_trace: Path, output: Path) -> dict:
    ...
    rtx_path = output / "rtx_reduction_path_trace.raw"; amd_path = output / "amd_reduction_path_trace.raw"
    ...
    receipt = {
        "schema": 1, "experiment": "n0_reduction_path_trace_result_ingestion", "status": "PASS",
        "source_archive": str(archive), "source_archive_sha256": archive_sha256(archive),
        "rtx_device_name": manifest["device_name"], "rtx_trace_sha256": sha256(trace),
...
def main() -> int:
    repo = Path(__file__).resolve().parents[1]; parser = argparse.ArgumentParser()
    parser.add_argument("archive", type=Path); parser.add_argument("--amd-trace", type=Path, default=repo / "results/20260901_133000_n0_reduction_path_trace/amd/reduction_path_trace.raw"); parser.add_argument("--output", type=Path); args = parser.parse_args()
    output = args.output or repo / "results" / f"{datetime.now():%Y%m%d_%H%M%S}_n0_reduction_path_cross_vendor"
    try:
