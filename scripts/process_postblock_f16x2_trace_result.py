#!/usr/bin/env python3
"""Strictly ingest the RTX post-block packed-F16 trace result."""
...
try:
    ...
    from scripts.instrument_postblock_f16x2_trace import TRACE_BYTES
except ModuleNotFoundError:
    from analyze_postblock_f16x2_trace import analyze_bytes
    from instrument_postblock_f16x2_trace import TRACE_BYTES


EXPECTED_REVISION = "v1_postblock_exact_frame1_selected_cta_f16x2_trace"
...
def process(archive: Path, output: Path, amd_trace: Path) -> dict:
    ...
    checks = {
        ...
        "experiment": manifest.get("experiment") ==
            "rtx_postblock_exact_frame1_selected_cta_f16x2_trace",
        "status": manifest.get("status") == "PASS",
        "classification": manifest.get("classification") == "RTX_POSTBLOCK_F16X2_ORACLE",
    ...
    (output / "comparison.json").write_text(json.dumps(comparison, indent=2) + "\n",
                                               encoding="utf-8")
    receipt = {
        "schema": 1, "experiment": "postblock_f16x2_trace_result_ingestion",
        "status": "PASS", "classification": "POSTBLOCK_F16X2_ORACLE_ACCEPTED",
...
def main() -> int:
    ...
    parser.add_argument("--amd-trace", type=Path,
                        default=Path("results/20260904_800000_postblock_exact_f16x2_trace/amd_trace.raw"))
