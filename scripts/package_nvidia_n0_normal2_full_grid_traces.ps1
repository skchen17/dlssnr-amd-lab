[CmdletBinding()]
param([string]$TraceRoot = 'results\20260904_160000_n0_normal2_full_grid_traces')
...
$stage = Join-Path $repo "deliverables\n0_normal2_full_grid_traces_reference_$stamp"
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'run_nvidia_n0_normal2_full_grid_traces.ps1') -Destination $scriptsDir
Copy-Item -LiteralPath (Join-Path $repo 'N0_NORMAL2_FULL_GRID_REFERENCE_README.md') -Destination (Join-Path $stage 'README.md')
...
$manifest = [ordered]@{
    schema = 1
    experiment = 'rtx_n0_normal2_full_grid_traces_package'
