[CmdletBinding()]
param([string]$TraceRoot = 'results\20260904_770000_postblock_exact_e4m3_mov_trace')
...
$stage = Join-Path $repo "deliverables\postblock_e4m3_mov_trace_reference_$stamp"
...
Copy-Item -LiteralPath (Join-Path $trace 'post_block_e4m3_mov_trace_rtx_v87.ptx') -Destination (Join-Path $payload 'post_block_e4m3_mov_trace.ptx')
...
Copy-Item -LiteralPath (Join-Path $repo 'POSTBLOCK_E4M3_MOV_TRACE_REFERENCE_README.md') -Destination (Join-Path $stage 'README.md')
$hashes = [ordered]@{}
...
[ordered]@{
    schema = 1
    package_revision = 'v1_postblock_exact_frame1_selected_cta_e4m3_mov_trace'
    experiment = 'rtx_postblock_exact_frame1_selected_cta_e4m3_mov_trace_package'
    result_stem = 'postblock_e4m3_mov_trace'
    result_experiment = 'rtx_postblock_exact_frame1_selected_cta_e4m3_mov_trace'
    result_classification = 'RTX_POSTBLOCK_E4M3_MOV_ORACLE'
    ptx_file = 'post_block_e4m3_mov_trace.ptx'
    ...
    families = $instrumentation.families
