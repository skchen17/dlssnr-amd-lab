...
if ((Get-Item -LiteralPath $raw).Length -ne 65536) {
    throw "expected a 65536-byte exhaustive RTX oracle: $raw"
}
& (Join-Path $PSScriptRoot 'build_all.ps1') -Only fp8_e4m3_probe
if ($LASTEXITCODE -ne 0) { throw "build failed: $LASTEXITCODE" }
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$result = Join-Path $repo "results\${stamp}_amd_fp16_e4m3_rtx_oracle"
New-Item -ItemType Directory -Path $result | Out-Null
$exe = Join-Path $repo 'build\fp8_e4m3_probe.exe'
$json = Join-Path $result 'fp16_e4m3_comparison.json'
& $exe --half-oracle $raw $json *> (Join-Path $result 'stdout.log')
...
$pass = $exitCode -eq 0 -and $run -and $run.status -eq 'PASS' `
    -and $run.mismatches -eq 0 -and $run.finite_mismatches -eq 0 `
    -and $run.infinity_mismatches -eq 0 -and $run.nan_mismatches -eq 0 `
