#!/usr/bin/env python3
"""Select sine-correction segments that safely fix normal1 FP16 boundaries."""
...
def make(base_model: dict, trace_root: Path) -> dict:
    ...
    if any(array.shape != (SAMPLES, 2) for array in (rtx_boundary, amd_boundary, rtx_sin, amd_sin)):
        raise ValueError("trace shape mismatch")
    if not np.array_equal(rtx_sin[:, 0], amd_sin[:, 0]):
        raise ValueError("sine inputs differ")
    ...
    position = np.float32(rtx_sin[:, 0].view("<f4") * scale)
    segment = np.minimum(position.astype(np.int32), segments - 1)
    local = np.float32(position - segment.astype("<f4"))
    corrected_sin = evaluate(amd_sin[:, 1].view("<f4"), coefficients, segment, local)
    corrected_normal = np.float32(
        amd_boundary[:, 0].view("<f4") * corrected_sin
    ).astype("<f2").view("<u2")
    ...
    return {
        "schema": 1,
        "experiment": f"nvidia_sm120_sin_boundary_safe_{segments}_segment_degree{degree}",
        "status": "PASS",
        "classification": "FULL_GRID_NORMAL1_BOUNDARY_CONSERVATIVE_SIN_CORRECTION_CANDIDATE",
