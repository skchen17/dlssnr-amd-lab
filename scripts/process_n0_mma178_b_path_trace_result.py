#!/usr/bin/env python3
"""Safely ingest an RTX N0 MMA-178 B-path trace and compare it with RX."""
...
try:
    ...
    from scripts.instrument_n0_mma178_b_path_trace import STAGES, TRACE_BYTES
    from scripts.process_slot3_mma_trace_result import archive_sha256, normalized_member
except ModuleNotFoundError:
    from analyze_n0_mma178_b_path_trace import analyze, sha256
    from instrument_n0_mma178_b_path_trace import STAGES, TRACE_BYTES
    from process_slot3_mma_trace_result import archive_sha256, normalized_member


EXPECTED_EXPERIMENT = "rtx_n0_mma178_b_path_trace"
MAX_MEMBERS = 32
...
def inspect_archive(zf: zipfile.ZipFile) -> tuple[zipfile.ZipInfo, zipfile.ZipInfo]:
    ...
    manifests = [(p, i) for p, i in files if p.name == "manifest.json"]
    traces = [(p, i) for p, i in files if p.name == "mma178_b_path_trace.raw"]
    if len(manifests) != 1 or len(traces) != 1:
        raise ValueError("archive must contain exactly one manifest.json and mma178_b_path_trace.raw")
    ...
    if manifest_info.file_size > 1024 * 1024 or trace_info.file_size != TRACE_BYTES:
        raise ValueError("manifest or trace size is invalid")
    return manifest_info, trace_info


def require_manifest(manifest: dict, trace: bytes) -> None:
    expected_stages = [
        {"index": i, "name": name, "register": register}
        for i, (name, _, register) in enumerate(STAGES)
    ]
    checks = {
        ...
        "launch": manifest.get("grid") == [1, 1, 1] and manifest.get("block") == [32, 1, 1] and manifest.get("kernel_launched") is True,
...
def process_archive(archive: Path, amd_trace: Path, output: Path) -> dict:
    ...
    rtx_path, amd_path = output / "rtx_mma178_b_path_trace.raw", output / "amd_mma178_b_path_trace.raw"
    ...
    receipt = {
        "schema": 1, "experiment": "n0_mma178_b_path_trace_result_ingestion", "status": "PASS",
...
def main() -> int:
    ...
    parser.add_argument("--amd-trace", type=Path, default=repo / "results/20260901_173000_n0_zero_mma178_b_path_trace/amd/mma178_b_path_trace.raw")
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    output = args.output or repo / "results" / f"{datetime.now():%Y%m%d_%H%M%S}_n0_mma178_b_path_cross_vendor"
    try:
