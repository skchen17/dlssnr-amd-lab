...
from scripts.process_postblock_f16x2_trace_result import EXPECTED_OUTPUT, process


class ProcessPostblockF16x2TraceResultTests(unittest.TestCase):
    def test_accepts_valid_result(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            trace = bytearray(TRACE_BYTES)
            for offset in range(0, TRACE_BYTES, 20):
                trace[offset + 16:offset + 20] = (1).to_bytes(4, "little")
            trace = bytes(trace)
            ...
            manifest = {
                ...
                "experiment": "rtx_postblock_exact_frame1_selected_cta_f16x2_trace",
                "status": "PASS", "classification": "RTX_POSTBLOCK_F16X2_ORACLE",
