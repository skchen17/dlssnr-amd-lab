...
def section(name, wmma=1, vgpr=20, lds=0, byte_store=False):
...
def test_audit_requires_all_stage_specific_kernels():
    text = ''.join(section(f'_Z_{family}ILi{channel}E',
                           byte_store=family == 'stage_qkv_pack_e4_from_fp16',
                           wmma=0 if family in ('stage_qkv_norm_fp16_inplace',
                                                'stage_qkv_pack_e4_from_fp16') else 1)
    ...
    text += ''.join(section(f'_Z_{family}ILi32E',
                            byte_store=family == 'stage_qkv_pack_e4_from_fp16',
                            wmma=0 if family in ('stage_qkv_norm_fp16_inplace',
                                                 'stage_qkv_pack_e4_from_fp16') else 1) for family in
                    ('stage_qkv_project_fp8', 'stage_qkv_norm_fp16_inplace',
                     'stage_qkv_pack_e4_from_fp16',
    ...
    assert result['all_standard_qkv_pack_kernels_publish_software_e4m3_bytes']
