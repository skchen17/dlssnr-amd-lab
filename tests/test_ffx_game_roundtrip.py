...
from analyze_ffx_game_roundtrip import analyze


def fixture(root):
    root.mkdir()
    output = root / 'output_roundtrip'; output.mkdir()
    width, height = 8, 4
    raw = b''.join(struct.pack('<4e', x / 8, y / 4, (x + y) / 12, 1)
    ...
    events = [dict(common, event='observation_idle'),
              dict(common, event='boundary_output_arm', write_back_performed=True),
              ...
              dict(common, event='boundary_output_recorded', isolated_host_only=False,
                   write_back_performed=True, game_frame_capture_verified=False),
              dict(common, event='boundary_output_submitted', isolated_host_only=False,
                   write_back_performed=True, game_frame_capture_verified=False,
                   submitted_identity_matches=1, identity_aliases=1, closed_identity_aliases=1,
                   raw_pointer_match=True),
...
def test_live_roundtrip_pass(tmp_path):
    fixture(tmp_path / 'run')
    report = analyze(tmp_path / 'run')
    assert report['status'] == 'GAME_OUTPUT_ROUNDTRIP_PASS'
    assert report['game_writeback_verified'] and report['write_back_performed']
    assert not report['replacement_pixels_supplied'] and not report['dlss_nr_verified']


def test_rejected_startup_boundary_is_retained(tmp_path):
    root = tmp_path / 'run'; events, _, _ = fixture(root)
    events.insert(2, dict(events[2], lifecycle_match=False))
    (root / 'session.jsonl').write_text('\n'.join(json.dumps(e) for e in events) + '\n')
    assert analyze(root)['status'] == 'GAME_OUTPUT_ROUNDTRIP_PASS'


@pytest.mark.parametrize('mutation,match', [
    ('duplicate', 'order/count'), ('wrong_order', 'order/count'), ('no_write', 'provenance'),
    ('replace', 'provenance'), ('not_fenced', 'provenance'), ('nr', 'provenance'),
...
def test_event_failures(tmp_path, mutation, match):
    ...
    elif mutation == 'no_write': events[-3]['write_back_performed'] = False
    elif mutation == 'replace': events[-1]['replacement_pixels_supplied'] = True
...
@pytest.mark.parametrize('mutation,match', [('truncate', 'byte count'), ('nan', 'non-finite'),
                                             ('constant', 'constant'), ('metadata', 'metadata'),
                                             ('tamper', 'provenance')])
def test_artifact_failures(tmp_path, mutation, match):
    root = tmp_path / 'run'; _, metadata, raw = fixture(root)
    path = root / 'output_roundtrip' / 'boundary_output.raw'
    if mutation == 'truncate': path.write_bytes(raw[:-2])
        ...
        metadata['write_back_performed'] = False
        (root / 'output_roundtrip' / 'metadata.json').write_text(json.dumps(metadata))
    else:
